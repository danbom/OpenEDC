...
export async function init() {
  ...
  for (const locale of Object.values(locales)) {
    await registerTranslationFile(
      locale,
      ioHelper.getBaseURL() + "/internationalization/" + locale + ".json"
    );
  }
  ...
  if (Object.values(locales).includes(browserLocale))
...
export async function registerTranslationFile(locale, url) {
  ...
  if (locale == currentLocale)
    Object.assign(translations, await getTranslations(url));
}
...
async function loadTranslations(locale) {
  if (locale != currentLocale) {
    ...
    for (const translationFile of translationFiles) {
      if (translationFile.locale == locale)
        Object.assign(translations, await getTranslations(translationFile.url));
    }
  }
...
export function localize(node = document) {
  node
    ...
    .forEach(
      (element) =>
        (element.textContent = getTranslation(element.getAttribute("i18n")))
    );
  node
    .querySelectorAll("[i18n-html]")
    .forEach(
      (element) =>
        (element.innerHTML = getTranslation(element.getAttribute("i18n-html")))
    );
  node
    .querySelectorAll("[i18n-ph]")
    .forEach(
      (element) =>
        (element.placeholder = getTranslation(element.getAttribute("i18n-ph")))
    );
  node
    .querySelectorAll("[i18n-href]")
    .forEach(
      (element) =>
        (element.href = getTranslation(element.getAttribute("i18n-href")))
    );
}
...
export async function setInitialLocale() {
  const initialLocale = localesInODM.includes(browserLocale)
    ? browserLocale
    : localesInODM[0];
